...
class EO_Ensemble():
    def __init__(self, inputtime, savepath, inputpath, scenario):
        ...
        self.var_dict = {
            'Power Decay': 2,
            'First Order': 1,
            'Parallel First Order': 3
        }
        self.labels_dict = {
            'Power Decay': ["Decay rate (k) (h^-1)", "Decay order (n) dimensionless"],
            'First Order': ["Decay rate (k) (h^-1)"],
            'Parallel First Order': ["Ratio of slow to fast decay (w)", "Decay rate 1 (k1) (h^-1)",
                                     "Decay rate 2 (k2) (h^-1)"]
        }
        self.solver_dict = {
            'Power Decay': 'Unbound',
    ...
    def import_data(self):
        ...
        self.X_cal, X_test, self.Y_cal, Y_test = train_test_split(X, Y, test_size=0.1, shuffle=True)
    ...
    def power_params_drop(self):
        delete_indices = [np.argwhere(self.SSE_ensemble_params['Power Decay'][:, 1] <= 0.0000001)]
        self.SSE_ensemble_params['Power Decay'] = np.delete(self.SSE_ensemble_params['Power Decay'], delete_indices,
                                                            axis=0)
        self.test_MSE['Power Decay'] = np.delete(self.test_MSE['Power Decay'], delete_indices, axis=0)
        self.test_r2['Power Decay'] = np.delete(self.test_r2['Power Decay'], delete_indices, axis=0)
        self.train_MSE['Power Decay'] = np.delete(self.train_MSE['Power Decay'], delete_indices, axis=0)
        self.train_r2['Power Decay'] = np.delete(self.train_r2['Power Decay'], delete_indices, axis=0)
        return

    def pfo_params_drop(self):
        delete_indices = [np.argwhere(self.SSE_ensemble_params['Parallel First Order'][:, 1] <= 0.0000001)]
        self.SSE_ensemble_params['Parallel First Order'] = np.delete(self.SSE_ensemble_params['Parallel First Order'],
                                                                     delete_indices,
                                                                     axis=0)
        self.test_MSE['Parallel First Order'] = np.delete(self.test_MSE['Parallel First Order'], delete_indices, axis=0)
        self.test_r2['Parallel First Order'] = np.delete(self.test_r2['Parallel First Order'], delete_indices, axis=0)
        self.train_MSE['Parallel First Order'] = np.delete(self.train_MSE['Parallel First Order'], delete_indices,
                                                           axis=0)
        self.train_r2['Parallel First Order'] = np.delete(self.train_r2['Parallel First Order'], delete_indices, axis=0)
        return
    ...
    def overfitting_check(self, model):
        ...
        # check 1 r2:
        check_r2_best = (np.max(self.test_r2[model]) - np.max(self.train_r2[model])) / (1-np.max(self.train_r2[model]))
        check_r2_worst = (np.percentile(self.test_r2[model],75) - np.percentile(self.train_r2[model],75)) / (1-np.percentile(self.train_r2[model],75))

        # check 2:
        check_mse_best = (np.min(self.test_MSE[model]) - np.min(self.train_MSE[model])) / np.min(self.train_MSE[model])
        check_mse_worst = (np.percentile(self.test_MSE[model],25) - np.percentile(self.train_MSE[model],25)) / np.percentile(self.train_MSE[model],25)

        if check_mse_best > 0.5 or check_mse_worst > 0.5 or check_r2_worst < -0.5 or check_r2_best < -0.5:
            self.convergence_checker[model] = False
    ...
    def fo_targets(self):
        ...
        CI_idx = np.argwhere(self.test_MSE['First Order'] <= np.percentile(self.test_MSE['First Order'], 25)).flatten()
        ...
        self.targets['First Order'] = pd.DataFrame({'Optimum Decay': np.array(opt_targets).flatten(),
        ...
        self.min_params['First Order']=[np.min(params),self.test_MSE['First Order'][np.argwhere(self.SSE_ensemble_params['First Order']==np.min(params))]]
        self.max_params['First Order']=[np.max(params),self.test_MSE['First Order'][np.argwhere(self.SSE_ensemble_params['First Order']==np.max(params))]]
        return

    def power_targets(self):
        times = np.arange(3, 25, 3)
        CI_idx = np.argwhere(self.test_MSE['Power Decay'] <= np.percentile(self.test_MSE['Power Decay'], 25)).flatten()
        ...
        min_decay_targets = []
        for t in times:
            opt_targets.append((0.3 ** (1 - n_opt) - (n_opt - 1) * k_opt * t) ** (1 / (1 - n_opt)))
            targets = (0.3 ** (1 - n) - (n - 1) * k * t) ** (1 / (1 - n))
            max_decay_targets.append(np.max(targets))
            min_decay_targets.append(np.min(targets))

        self.targets['Power Decay'] = pd.DataFrame({'Optimum Decay': np.array(opt_targets),
                                                    'Maximum Decay': np.array(max_decay_targets),
                                                    'Minimum Decay': np.array(min_decay_targets)}, index=times)
        ...
        if np.isnan(self.targets['Power Decay'][self.scenario].loc[self.inputtime])==True:
            self.convergence_checker['Power Decay'] = False
            return

        inp_target=(0.3 ** (1 - n) - (n - 1) * k * self.inputtime) ** (1 / (1 - n))
        n_min,k_min,n_max,k_max=n[np.argmin(inp_target)],k[np.argmin(inp_target)],n[np.argmax(inp_target)],k[np.argmax(inp_target)]
    ...
    def pfo_targets(self):
        times = np.arange(3, 25, 3)
        CI_idx = np.argwhere(self.test_MSE['Parallel First Order'] <= np.percentile(self.test_MSE['Parallel First Order'], 25)).flatten()
        ...
        min_decay_targets = []
        for t in times:
            opt_targets.append(fsolve(
                lambda C_0: w_opt * C_0 * np.exp(-1 * k1_opt * t) + (1 - w_opt) * C_0 * np.exp(-1 * k2_opt * t) - 0.3,
                0.3))

            targets = fsolve(lambda C_0: w * C_0 * np.exp(-1 * k1 * t) + (1 - w) * C_0 * np.exp(-1 * k2 * t) - 0.3,
                             np.array([0.3 for i in range(len(w))]))
            max_decay_targets.append(np.max(targets))
            min_decay_targets.append(np.min(targets))


        self.targets['Parallel First Order'] = pd.DataFrame({'Optimum Decay': np.array(opt_targets).flatten(),
                                                             'Maximum Decay': np.array(max_decay_targets),
                                                             'Minimum Decay': np.array(min_decay_targets)}, index=times)
        ...
        if self.targets['Parallel First Order'][self.scenario].loc[self.inputtime] < 0.3:
            self.convergence_checker['Parallel First Order'] = False
            return

        if self.targets['Parallel First Order'][self.scenario].loc[self.inputtime] > 100:
            self.convergence_checker['Parallel First Order'] = False
            return

        if np.isnan(self.targets['Parallel First Order'][self.scenario].loc[self.inputtime])==True:
            self.convergence_checker['Parallel First Order'] = False
            return

        inp_target = fsolve(lambda C_0: w * C_0 * np.exp(-1 * k1 * self.inputtime) + (1 - w) * C_0 * np.exp(-1 * k2 * self.inputtime) - 0.3,
                             np.array([0.3 for i in range(len(w))]))
        w_min,k1_min, k2_min, w_max,k1_max, k2_max = w[np.argmin(inp_target)], k1[np.argmin(inp_target)],k2[np.argmin(inp_target)],\
                                                     w[np.argmax(inp_target)],k1[np.argmax(inp_target)],k2[np.argmax(inp_target)]
    ...
    def First_Order_Model(self):
        ...
        params, train_preds, train_mse, train_r2, test_preds, test_mse, test_r2 = self.EO_Ensemble_SSE_unbound(
            self.X_cal, self.Y_cal, self.f0_test,
            self.f_test,
            self.t_test)
    ...
    def Power_Decay_Model(self):
        ...
        self.bounds = self.bounds_dict[eqn]
        self.guess = self.guess_dict[eqn]
        params, train_preds, train_mse, train_r2, test_preds, test_mse, test_r2 = self.EO_Ensemble_SSE_bound(self.X_cal,
                                                                                                             self.Y_cal,
                                                                                                             self.f0_test,
                                                                                                             self.f_test,
                                                                                                             self.t_test)
    ...
    def EO_Ensemble_SSE_unbound(self, X_cal, Y_cal, f0_test, f_test, t_test):
        ...
        for i in range(0, self.ensemble_size):
            ...
            X_train, X_val, Y_train, Y_val = train_test_split(self.X_cal, Y_cal, test_size=0.25, shuffle=True)
            f0 = pd.Series(X_train['ts_frc'].values).to_numpy()
            t = pd.Series(X_train['se4_lag'].values).to_numpy()
            f = Y_train.to_numpy()

            sol = minimize(self.cost_check_SSE, guess, method='Powell', args=(f0, t, f),
                           options={'maxiter': 600})
            xopt = sol.x
        ...
        return ens_params, np.array(f_pred), np.array(ens_MSE_train), np.array(ens_r2_train), np.array(
            f_pred_test), np.array(ens_MSE_test), np.array(ens_r2_test)
    ...
    def EO_Ensemble_SSE_bound(self, X_cal, Y_cal, f0_test, f_test, t_test):
        ...
        for i in range(0, self.ensemble_size):
            guess = np.random.sample(self.nvar) * self.guess
            X_train, X_val, Y_train, Y_val = train_test_split(X_cal, Y_cal, test_size=0.25, shuffle=True)
            f0 = pd.Series(X_train['ts_frc'].values).to_numpy()
            t = pd.Series(X_train['se4_lag'].values).to_numpy()
            f = Y_train.to_numpy()

            sol = minimize(self.cost_check_SSE, guess, method='TNC', args=(f0, t, f), bounds=self.bounds)
            xopt = sol.x
    ...
    def k_n_fig(self, solver):
        ...
        if len(self.labels_dict[solver]) == 1:
            fig = plt.figure()
            plt.scatter(self.SSE_ensemble_params[solver],self.test_MSE[solver],marker='o', edgecolors='k',facecolors='none',linewidths=0.5)
            plt.scatter(self.opt_params[solver],np.min(self.test_MSE[solver]),marker='*',s=100,c='#0000ff',label='Optimum Decay Parameters')
            plt.scatter(self.CI_params[solver],self.test_MSE[solver][np.argwhere(self.test_MSE[solver]<=np.percentile(self.test_MSE[solver],25))],
                        edgecolors='#0000ff',facecolors='none',linewidths=0.5,label='75% Confidence Region')
            plt.scatter(self.min_params[solver][0],np.min(self.min_params[solver][1]),marker='*',s=100,c='#00ff00',label='Minimum Decay Parameters')
            plt.scatter(self.max_params[solver][0], np.min(self.max_params[solver][1]), marker='*', s=100, c='#ff0000',
            ...
            fig = plt.figure()
            plt.scatter(self.SSE_ensemble_params[solver][:,0], self.SSE_ensemble_params[solver][:,1],marker='o', edgecolors='k',facecolors='none',linewidths=0.5)
            plt.scatter(self.opt_params[solver][0],self.opt_params[solver][1],marker='*',s=100,c='#0000ff',label='Optimum Decay Parameters')
            if len(self.SSE_ensemble_params[solver])>1:
                plt.scatter(self.CI_params[solver][0], self.CI_params[solver][1],edgecolors='#0000ff', facecolors='none', linewidths=0.5,label='75% Confidence Region')
                plt.scatter(self.min_params[solver][0], self.min_params[solver][1], marker='*', s=100, c='#00ff00',
                            label='Minimum Decay Parameters')
                plt.scatter(self.max_params[solver][0], self.max_params[solver][1], marker='*', s=100, c='#ff0000',
                            label='Maximum Decay Parameters')
            plt.title(solver)
            plt.xlabel(self.labels_dict[solver][0])
            ...
            ax.scatter(self.SSE_ensemble_params[solver][:, 1], self.SSE_ensemble_params[solver][:, 2],marker='o', edgecolors='k',facecolors='none',linewidths=0.5)
            ax.scatter(self.opt_params[solver][1], self.opt_params[solver][2], marker='*', s=100,c='#0000ff',label='Optimum Decay Parameters')
            if len(self.SSE_ensemble_params[solver]) > 1:
                plt.scatter(self.CI_params[solver][0][1], self.CI_params[solver][0][2], edgecolors='#0000ff', facecolors='none',
                            linewidths=0.5, label='75% Confidence Region')
                plt.scatter(self.min_params[solver][1], self.min_params[solver][2], marker='*', s=100, c='#00ff00',
                            label='Minimum Decay Parameters')
                plt.scatter(self.max_params[solver][1], self.max_params[solver][2], marker='*', s=100, c='#ff0000',
                            label='Maximum Decay Parameters')
            ...
            ax.scatter(self.SSE_ensemble_params[solver][:, 0], self.SSE_ensemble_params[solver][:, 1],marker='o', edgecolors='k',facecolors='none',linewidths=0.5)
            ax.scatter(self.opt_params[solver][0], self.opt_params[solver][1], marker='*', s=100,c='#0000ff')
            if len(self.SSE_ensemble_params[solver]) > 1:
                plt.scatter(self.CI_params[solver][0][0], self.CI_params[solver][0][1], edgecolors='#0000ff', facecolors='none')
                plt.scatter(self.min_params[solver][0], self.min_params[solver][1], marker='*', s=100, c='#00ff00')
                plt.scatter(self.max_params[solver][0], self.max_params[solver][1], marker='*', s=100, c='#ff0000')
            ...
            ax.scatter(self.SSE_ensemble_params[solver][:, 0], self.SSE_ensemble_params[solver][:, 2],marker='o', edgecolors='k',facecolors='none',linewidths=0.5)
            ax.scatter(self.opt_params[solver][0], self.opt_params[solver][2], marker='*', s=100,c='#0000ff')
            if len(self.SSE_ensemble_params[solver]) > 1:
                plt.scatter(self.CI_params[solver][0][0], self.CI_params[solver][0][2], edgecolors='#0000ff', facecolors='none',
                            linewidths=0.5)
                plt.scatter(self.min_params[solver][0], self.min_params[solver][2], marker='*', s=100, c='#00ff00')
                plt.scatter(self.max_params[solver][0], self.max_params[solver][2], marker='*', s=100, c='#ff0000')
            ax.set_xlabel(self.labels_dict[solver][0])
            ax.set_ylabel(self.labels_dict[solver][2])
    ...
    def confidence_assess(self):
        ...
        if n_obs>=150:
            ...
            self.confidence_reason['Observations']='At least 150 observations sent for analysis'
        elif n_obs>=100:
            self.confidence['Observations'] = 'Moderate'
            self.confidence_reason['Observations'] = 'Between 100 and 150 observations sent for analysis'
        else:
            self.confidence['Observations'] = 'Low'
            self.confidence_reason['Observations'] = 'Fewer than 100 observations sent for analysis'
        ...
        hh_p_levene=[]
        for i in range(100):
            hh_check,drop,ts_check,drop2=train_test_split(hh_frc_combined,ts_frc_combined,test_size=0.1)
        ...
        mins=[np.min(ts_p_ks2),np.min(ts_p_levene),np.min(hh_p_ks2),np.min(hh_p_levene)]
        if np.min(mins)>0.10:
            self.confidence['Stability']='High'
            self.confidence_reason['Stability']='Variance and distribution of household and tapstand FRC are not significantly different at a p-value level of 0.10'
        elif np.min(mins)>0.05:
            self.confidence['Stability'] = 'Moderate'
            self.confidence_reason[
                'Stability'] = 'Variance and distribution of household and tapstand FRC are not significantly different at a p-value 0.05 but are significantly different at a p-value of 0.10'
        else:
            self.confidence['Stability'] = 'Low'
            self.confidence_reason[
                'Stability'] = 'Variance or distribution of household and tapstand FRC are significantly different stable at a p-value level of 0.05'

        #3 - Check histogram densities
        bin_centre=np.arange(3,30,3)
        bins=bin_centre-1.5
        hist=np.histogram(np.append(self.X_cal['se4_lag'].values,self.t_test).flatten(),bins=bins,density=True)[0]
        ...
        if density_hist>=np.percentile(hist,75):
            ...
            self.confidence_reason['Histogram'] = 'Target storage duration and sampling durations are very similar (density of observations around storage target is higher than the upper quartile density)'
        elif  density_hist>=np.percentile(hist,50):
            self.confidence['Histogram'] = 'Moderate'
            self.confidence_reason['Histogram'] = 'Target storage duration are mostly similar (density of observations around storage target is higher than the median density)'
        elif density_hist>0:
            self.confidence['Histogram'] = 'Low'
            self.confidence_reason[
                'Histogram'] = 'Fewer than 25% of observations have sampling durations similar to the target storage duration'
        else:
            self.confidence['Histogram'] = 'Low'
            self.confidence_reason[
                'Histogram'] = 'No observations have sampling durations similar to the target storage duration'

        #4 - Check histogram uniformity

        uniform=[1/len(np.arange(3,self.inputtime+1,3)) for i in np.arange(3,self.inputtime+1,3)]
        ...
        if p>=0.5:
            ...
            self.confidence_reason['Uniformity'] = 'Uniform distribution of samples up to target storage duration'
        elif p >= 0.05:
            self.confidence['Uniformity'] = 'Moderate'
            self.confidence_reason['Uniformity'] = 'Mostly uniform distribution of samples up to target storage duration'
        else:
            self.confidence['Uniformity'] = 'Low'
            self.confidence_reason[
                'Uniformity'] = 'Minimal uniformity of distribution of samples up to target storage duration'
        ...
        confidence_df=pd.DataFrame({'Confidence Level':self.confidence.values(),'Confidence Reason':self.confidence_reason.values()},index=self.confidence.keys())
    ...
    def target_fig(self,target):
        ...
        ax.plot(self.targets[self.model].index,self.targets[self.model][self.scenario],c='#0000ff',label=self.scenario+' FRC Target',zorder=10)
        secax=ax.twinx()
        secax.hist(np.append(self.X_cal['se4_lag'].values,self.t_test).flatten(),zorder=100,alpha=0.25,facecolor='purple')
        ...
        pred=self.f_dict[self.model](params,np.array([target for i in range(len(times))]),times)
        ...
        self.targetdecay_base_64_pngData = base64.b64encode(StringIOBytes_targetdecay.read())
        plt.close()
        return
    ...
    def back_check_fig(self, target):
        ...
        test_df = pd.DataFrame({'se4_lag': self.t_test, 'ts_frc': self.f0_test, 'hh_frc': self.f0_test})
        ...
        if (sphere_total_cal + sphere_total_test)>0:
            sphere_safe_percent = np.round((sphere_safe_cal + sphere_safe_test) / (sphere_total_cal + sphere_total_test)*100,decimals=1)
            self.sphere_text = 'Existing Guidelines, 0.2-0.5 mg/L, ' + str(sphere_safe_cal + sphere_safe_test) + ' of ' \
                          + str(sphere_total_cal + sphere_total_test) + ', ' + \
                          str(np.round(sphere_safe_percent, decimals=1)) + '% household water safety success rate'
        else:
        ...
        if (target_cal_total + target_total_test)>0:
            target_safe_percent = (target_cal_safe + target_test_safe) / (target_cal_total + target_total_test)*100
            self.target_text = 'Proposed Guidelines, ' + str(np.round(target, decimals=2)) + '-' + str(
                np.round(target + 0.2, decimals=2)) + ' mg/L, ' + \
                          str(target_cal_safe + target_test_safe) + ' of ' + str(target_cal_total + target_total_test) + \
                          ', ' + str(
                np.round(target_safe_percent, decimals=1)) + '% household water safety success rate'
        else:
            self.target_text='Proposed Guidelines, ' + str(np.round(target, decimals=2)) + '-' + str(
                np.round(target + 0.2, decimals=2)) + ' mg/L, Water safety success rate unavailable (no samples with similar storage duration and target)'

        box_props=dict(boxstyle='square',facecolor='white',alpha=0.5)

        backcheck_fig,ax = plt.subplots(figsize=(8,5.5))
        ax.set_title('SWOT Engineering Optimization Model - Empirical Back-Check at ' + str(time_lb) + '-' +
                  str(time_ub) + 'h follow-up (average ' + str(np.round(s, decimals=1)) +
                  ', n=' + str(n) + ')\nCode Version: ' + self.version,fontsize=10)
        #ax.text(0.01,0.95,sphere_text+'\n'+target_text,transform=ax.transAxes,bbox=box_props,fontsize=8,wrap=True)
        ax.scatter(test_df['ts_frc'], test_df['hh_frc'], marker='o', edgecolors='#0000ff',facecolors='none',linewidths=0.5)
        ax.scatter(cal_df['ts_frc'], cal_df['hh_frc'], marker='o', edgecolors='#0000ff',facecolors='none',linewidths=0.5)
        ...
        self.backcheck_base_64_pngData = base64.b64encode(StringIOBytes_backcheck.read())
        plt.close()
        return
    ...
    def generate_html_report(self,confidence):
        ...
        with tag("p",klass="storage_duration"):
            text("Average Storage Duration: "+str(np.round(st_dur,decimals=0))+" hours and "+str(int((st_dur-np.round(st_dur,decimals=0))*60))+"minutes")

        with tag("div",id="target decay fig"):
            doc.stag("img",src=(self.savepath + "\\targets.png"))

        with tag("h2",klass="Header"):
            text("Anticipated household FRC after storage target for Tapstand FRC of "+str(self.target)+" mg/L")
    ...
    def run_EO(self):
        ...
        best_model_params=pd.Series(data=np.hstack((params)),index=[ 'Power Decay k', 'Power Decay n','First Order k', 'Parallel First Order w','Parallel First Order k1','Parallel First Order k2'])
        self.select_model()
